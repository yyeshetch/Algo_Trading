...
def _market_window_30min(trade_date: date | None) -> tuple[datetime, datetime]:
    """Session window for 30-min candles."""
    if trade_date is not None:
        session_start = datetime.combine(trade_date, datetime.min.time()).replace(hour=9, minute=15)
        session_end = datetime.combine(trade_date, datetime.min.time()).replace(hour=15, minute=30)
        return session_start, session_end
    ...
    previous_completed = current_boundary - timedelta(minutes=30)
    return session_start, previous_completed
...
def fetch_stock_30min_frame(
    ...
    """
    Fetch 30-min candles for spot + future (+ optional ATM CE/PE).
    Same structure as fetch_stock_15min_frame but with 30-minute interval.
    """
    uc = get_underlying_config(stock_name)
    settings = Settings.from_env(underlying=stock_name)
    resolver = InstrumentResolver(client, settings)
    from_dt, to_dt = _market_window_30min(trade_date)
    if to_dt < from_dt:
        return None
    effective_date = from_dt.date()
    ...
    spot_rows = client.historical_data(spot_token, from_dt, to_dt, interval="30minute")
    spot_df = _to_candle_df(spot_rows, "spot")
    if spot_df.empty:
        return None
    ...
    fut_token = int(fut_quote[symbols.fut_symbol]["instrument_token"])
    fut_df = _to_candle_df(
        client.historical_data(fut_token, from_dt, to_dt, interval="30minute", oi=True),
        "future",
        include_oi=True,
    )
    merged = spot_df.merge(fut_df, on="timestamp", how="inner")
    ...
    if include_options:
        try:
            ...
            pe_token = int(pe_quote[symbols.pe_symbol]["instrument_token"])
            ce_df = _to_candle_df(
                client.historical_data(ce_token, from_dt, to_dt, interval="30minute", oi=True),
                "call",
                include_oi=True,
            )
            pe_df = _to_candle_df(
                client.historical_data(pe_token, from_dt, to_dt, interval="30minute", oi=True),
                "put",
                include_oi=True,
            )
